...
export const useMotoStore = create<MotoState>((set) => ({
  ...
  updateMoto: (moto) =>
    set((state) => ({
      ...
      selectedMoto: state.selectedMoto?.id === moto.id ? moto : state.selectedMoto,
    })),
  
  removeMoto: (motoId) =>
    set((state) => ({
      motos: state.motos.filter((m) => m.id !== motoId),
      selectedMoto: state.selectedMoto?.id === motoId ? null : state.selectedMoto,
    })),
}));
