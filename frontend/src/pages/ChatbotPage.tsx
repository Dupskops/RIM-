...
const initialMsgs: Msg[] = [
  {
    id: "m1",
    from: "bot",
    text: "Hola, soy RIM. Puedo ayudarte con diagnósticos y dudas sobre tu moto.",
  },
];
          ...
          prev.map((c) =>
            c.id === conversationId
              ? {
                  ...c,
                  msgs,
                  title: response.data.conversacion.titulo || c.title,
                }
              : c
          )
        );
      ...
      <div className="max-w-3xl mx-auto">
            ...
            <div className="relative" ref={menuRef}>
              ...
              {showMenu && (
                <div className="absolute right-0 mt-2 w-56 bg-[var(--card)] rounded-md shadow-md p-2 z-20">
                  <button
                    className="w-full text-left px-3 py-2 rounded hover:bg-[var(--accent)]"
                    onClick={() => {
                      setShowMenu(false);
                      setConnected(true);
                    }}
        ...
        {showHistory && (
          <div className="fixed inset-0 z-30 flex items-center justify-center">
            ...
            <div
              ref={historyRef}
              className="relative w-full max-w-2xl mx-4 bg-[var(--card)] rounded-lg p-4 text-[var(--muted)] z-40"
            >
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold text-[var(--bg)]">
              ...
              <div className="max-h-80 overflow-y-auto divide-y divide-[rgba(255,255,255,0.03)]">
                {loadingHistory ? (
                  ...
                  conversations.map((conv) => (
                    ...
                    >
                      <div className="flex-shrink-0 w-10 h-10 rounded-md bg-[rgba(255,255,255,0.02)] flex items-center justify-center text-[var(--accent)] font-semibold">
                        {generateTitle(conv)
                          .split(" ")[0]
                          ?.slice(0, 2)
                          .toUpperCase()}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-semibold text-[var(--bg)] truncate">
                          {generateTitle(conv)}
                        </div>
                        <div className="text-xs text-[var(--muted)] truncate">
                          Último:{" "}
                          {conv.msgs.length
                            ? conv.msgs[conv.msgs.length - 1].text.length > 60
                              ? conv.msgs[conv.msgs.length - 1].text.slice(
                                  0,
                                  57
                                ) + "..."
                              : conv.msgs[conv.msgs.length - 1].text
                            : "Sin mensajes"}
                        </div>
                      </div>

                      <div className="ml-3 flex-shrink-0 flex flex-col items-end gap-1 text-right">
                        <div className="bg-[var(--accent)] text-[#071218] px-2 py-1 rounded-full text-xs font-medium">
                          {conv.msgs.length}
                        </div>
          ...
          <div className="md:col-span-2 flex flex-col bg-[var(--card)] rounded-xl overflow-hidden h-[70vh] md:h-[72vh]">
            ...
            >
              {(currentConv?.msgs || []).map((m: Msg) => (
                ...
                >
                  <div
                    className={`${m.from === "user" ? "bg-[var(--accent)] text-[#071218]" : "bg-[rgba(255,255,255,0.03)] text-[var(--color-2)]"} max-w-[80%] p-3 rounded-lg`}
                  >
              ...
              {loading && (
                <div className="flex justify-start">
                  <div className="bg-[rgba(255,255,255,0.03)] p-2 rounded-lg flex items-center gap-2">
                    ...
                    <span className="text-[var(--color-2)] text-sm">
                      RIM está escribiendo...
                    </span>
                  </div>
                </div>
              )}
                ...
                <div className="mb-3 mx-auto max-w-xl">
                  <div className="bg-[var(--card)] rounded-xl p-3 space-y-3">
                    <div className="flex items-center justify-between">
                      <h4 className="font-semibold text-[var(--bg)]">
                        Sugerencias
                      </h4>
                      <span className="text-xs text-[var(--color-2)]">
                        Interacción rápida
                      </span>
                    </div>
